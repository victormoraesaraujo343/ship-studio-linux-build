...
- **GitLab support** - Branches, pushes and merge requests follow your project's actual forge and remote instead of assuming GitHub and `origin`; `glab` drives GitLab the way `gh` drives GitHub.
- **A signal that could end your session** - A stop signal aimed at pid 0 reached the whole login session instead of the child process.
- **Linux onboarding** - Setup drives your distribution's package manager, and terminals spawn your real login shell — fish included — not a hard-coded one.
- **Telemetry off by default** - Analytics and error reporting ship disabled in this fork.
